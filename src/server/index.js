...
// Empacotado como função, e não como efeito de importar o módulo: quem chama
// (o `npm start` da versão web, ou o processo principal do Electron) decide a
// porta, onde vivem o .env/branding/UI, e se deve abrir um browser externo —
// o Electron mostra a sua própria janela, por isso não quer um browser a mais.
// Os defaults reproduzem exatamente o comportamento de sempre da versão web.
function criarServidor(opcoes = {}) {
    const {
        porta = Number(process.env.PORT) || 4711,
    ...
    function aplicarConfig(novaConfig) {
        config = novaConfig;
        baseDir = path.resolve(novaConfig.downloadDir);
    }

    const arranque = configStore.tentarCarregarConfig(envPath);
    ...
    app.post('/api/config', (req, res) => {
        try {
            aplicarConfig(configStore.gravar(envPath, req.body || {}));
            res.json({ ok: true });
        } catch (err) {
    ...
    app.post('/api/config/testar', async (req, res) => {
        let configTeste;
        try {
            configTeste = configStore.validar(envPath, req.body || {});
        } catch (err) {
    ...
    return new Promise((resolve, reject) => {
        const servidor = app.listen(porta, '127.0.0.1', () => {
            ...
            // nas duas plataformas. O Electron passa abrirBrowser:false — já tem a
            // sua própria janela.
            if (abrirBrowser) abrirNoBrowser(url);

            resolve({ app, servidor, url, fechar: () => servidor.close() });
        });
...
// `node src/server/index.js` (o que o `npm start` faz) continua a arrancar
// sozinho, exatamente como antes — só passou a ser a chamada explícita da
// função, em vez de código a correr como efeito colateral do require.
if (require.main === module) {
    criarServidor().catch(err => {
        console.error('\nNão foi possível arrancar o servidor:\n  ' + err.message + '\n');
        process.exit(1);
    });
}
